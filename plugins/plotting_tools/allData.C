...
void allData::Loop() {
  ...
  //else 
  //  cout<<"Sampling another sample"<<endl;
  ...
  fChain->SetBranchStatus("ElecTrkIso",1);
  fChain->SetBranchStatus("ElecECalIso",1);
  fChain->SetBranchStatus("ElecHCalIso",1);
  ...
  TH1F *h_selections[2];
  TH1F *h_Nelec[4], *h_Ngoodelec[4], *h_Nmuon[4], *h_Ngoodmuon[4];
  ...
  string histname[NUMHISTOS] = {"01_jet1et","02_jet2et","03_jetallet",
				...
				"51a_Nelecs","51b_Ngoodelecs",
				"52a_Nmuons","52b_Ngoodmuons",
  ...
  for (int tt = 0; tt < 4; ++tt) {
    ...
    h_Ngoodelec[tt] = new TH1F(histtitle[18],"",static_cast<int>(bins[tt][17]),0.,bins[tt][17]);
    h_Nmuon[tt]     = new TH1F(histtitle[19],"",static_cast<int>(bins[tt][18]),0.,bins[tt][18]);
    h_Ngoodmuon[tt] = new TH1F(histtitle[20],"",static_cast<int>(bins[tt][18]),0.,bins[tt][18]);
    h_elecEta[tt]   = new TH1F(histtitle[21],"",100,-5.,5.);
    h_muonEta[tt]   = new TH1F(histtitle[22],"",100,-5.,5.);
    
    binsize = 25.;// bins of 25 GeV 
    h_elecEt[tt]   = new TH1F(histtitle[23],"",static_cast<int>(bins[tt][21]/binsize),0.,bins[tt][21]);
    h_muonEt[tt]   = new TH1F(histtitle[24],"",static_cast<int>(bins[tt][22]/binsize),0.,bins[tt][22]);
  }
  ...
  h_Minv[0]    = new TH1F("h_pre_cuts_9_Minv",   "",1500/25,0,1500);
  //post cuts
  ...
  h_Minv[1]    = new TH1F("h_post_cuts_9_Minv",   "",1500/25,0,1500);
  ...
  for (Long64_t jentry=0; jentry<nentries;++jentry) {
    ...
    //double METzphi2 = atan2(METy,METx);
	...
	if ((ElecTrkIso[ielec]+ElecECalIso[ielec]+ElecHCalIso[ielec])/ElecPt[ielec]<cut_eleciso) {
    ...
    for (unsigned int imuon = 0; imuon < Nmuon; ++imuon) {
      if (MuonPt[imuon]>cut_muonpt&&MuonEta[imuon]<cut_muoneta) 
	if ((MuonTrkIso[imuon]+MuonECalIso[imuon]+MuonHCalIso[imuon])/MuonPt[imuon]<cut_muoniso) {
	  muonVeto[0] = false;
	  ++goodmuoncount;
	}
    }
    ...
    //    printf("dijet   finaljet   dphi   ht   mht   met   lepton\n");
    //    printf("%d   %d   %d   %d   %d   %d   %d\n",dijetselection,finaljet,dphiselection,htselection,mhtselection,metselection,leptonveto);
    ...
    mhtSelection[1] = nJetSelection[0]&&
      ...
      jet1EMFracSelection[0]&&jet2EMFracSelection[0]&&
      jet1metdphiSelection[0]&&jet2metdphiSelection[0]&&
      metSelection[0]&&htSelection[0]&&meffSelection[0]&&
      excessiveJetVeto[0]&&jet12dphiSelection[0]&&
      electronVeto[0]&&muonVeto[0];
	...
	if (MuonPt[imuon]>cut_muonpt&&MuonEta[imuon]<cut_muoneta) 
      ...
      //eleccount = 0;
      //for (unsigned int ielec = 0; ielec < Nelec; ++ielec) if(ElecEt[ielec]>cut_elecpt) {
      //	h_elecEt[3]->Fill(ElecEt[ielec]);
      //	eleccount++;}
      //h_Nelec[3]->Fill(eleccount);
      //
      //muoncount = 0;
      //for (unsigned int imuon = 0; imuon < Nmuon; ++imuon) if(MuonEt[imuon]>cut_muonpt) {
